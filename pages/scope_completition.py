...
from untilities.funkcje import full_magic, unique_1, unique_2, get_unique_countries
import datetime
...
def scope_page():
    ...
    full_magic(unique_2(przyklad_standard_group["Region"]),
        unique_1(przyklad_standard_group, przyklad_standard_group["Region"], unique_2(przyklad_standard_group["Region"])),
        "Region",
        "Scope Completition",
        f"Chart illustrating the Level of Scope Completition by region",
        highlight_index=list(przyklad_standard_group["Region"].unique()).index(option1) if option1 != "All" else None,
    ...
    full_magic(unique_2(unique_clusters),
        unique_1(przyklad_standard_group, przyklad_standard_group["Cluster"], unique_2(unique_clusters)),
        "Region",
        "Scope Completition",
        f"Chart illustrating the Level of Scope Completition by cluster",
        highlight_index=list(unique_clusters.unique()).index(option2) if option2 != "All" else None,
    ...
    full_magic(unique_2(unique_country),
        unique_1(przyklad_standard_group, przyklad_standard_group["Country"], unique_2(unique_country)),
        "Region",
        "Scope Completition",
        f"Chart illustrating the Level of Scope Completition by cluster",
        highlight_index=list(unique_country.unique()).index(option3) if option3 != "All" else None,
        filename=f"{today}_Scope_Completition_level_per_cluster.png"
        )
